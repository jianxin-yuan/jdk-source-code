...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    private static int ctlOf(int rs, int wc) {
        //rs  111,00000000000000000000000000000
    ...
    private void processWorkerExit(Worker w, boolean completedAbruptly) {
        ...
        if (runStateLessThan(c, STOP)) {
            // 正常完成,判断当前工作线程数是否小于最小线程数,如果是,创建一个非核心线程加入工作线程中
            // 只有非核心线程正常关闭或者 允许核心线程 超时关闭才会走这段逻辑
    ...
    private Runnable getTask() {
        ...
        for (; ; ) {
            ...
            //判断线程是否可以超时释放,因为没有特意设置字段标记线程是否为核心线程,所有是通过当前工作线程数量和核心线程数量的值比较来判断的
    ...
    public void execute(Runnable command) {
        ...
        if (isRunning(c) && workQueue.offer(command)) {
            int recheck = ctl.get();
            //如果线程池状态不是RUNNING,移除队列中的任务,同时执行拒绝策略
            if (!isRunning(recheck) && remove(command))
                reject(command);

            //表示线程池处于RUNNING,但是无工作线程,创建一个线程
            else if (workerCountOf(recheck) == 0)
                addWorker(null, false);

            //放入队列失败,尝试创建非核心线程,如果失败,表示线程池处于已饱和或已关闭,执行拒绝策略
